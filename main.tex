...
This paper describes a different solution to the constraints listed above, which does not affect the portability of Uproot or Awkward Array. We introduce a domain-specific language (DSL) for deserializing arbitrary data into Awkward Arrays that is runtime-interpreted but nearly as fast as compiled code. Unlike DSLs intended for physicists to write analysis code, this DSL is ``internal,'' only used to communicate between software packages: Uproot's task becomes one of expressing deserialization commands, and Awkward Array executes them.
